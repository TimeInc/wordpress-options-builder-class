...
class Page extends Panel {
	...
	public function build_parts() {
		...
		$ok_html_tags = wp_kses_allowed_html( 'post' );
		$ok_html_tags['li'] = array_merge( $ok_html_tags['li'], [
			'data-part' => true,
		]);
		$ok_html_tags = array_merge( $ok_html_tags, [
			'input' => [
				'id' 	=> true,
				'class' => true,
				'data-part' => true,
...
//		echo wp_kses( ob_get_clean(), $ok_html_tags );
		echo ob_get_clean();
	}
	...
	public function inline_styles_and_scripts() {
		...
		<?php
		$js = ob_get_clean();
		echo PHP_EOL . '<style>' . $css . '</style>' . PHP_EOL . '<script>' . $js . '</script>' . PHP_EOL;
	}
	...
	public function enqueue_dependencies() {
		...
		wp_add_inline_style( 'wpop-pure-forms', $this->inline_css() );
...
class Section {
	...
	public function echo_html() {
		...
		], HTML::tag( 'ul', [], HTML::tag( 'li', [], $section_content ) ) );
